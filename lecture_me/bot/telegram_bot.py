...
from lecture_me.models.data_models import Question, UserSession
...
class TelegramBot:
    def __init__(
        self, token: str, notes_service: NotesService, llm_service: LLMService
    ):
        self.token = token
    ...
    async def start_command(
        ...
        assert update.effective_user is not None, "User must be available"
        assert update.message is not None, "Message must be available"
    ...
    async def study_command(
        ...
        if not subjects:
            await update.message.reply_text(
                "No study materials found! Please make sure your notes directory "
                "contains subjects with topics and markdown files."
            )
            return
        ...
        await update.message.reply_text(
            "Choose a subject to study:", reply_markup=reply_markup
        )
    ...
    async def stats_command(
        ...
        if session.questions_answered == 0:
            await update.message.reply_text(
                "You haven't answered any questions yet! Use /study to start learning."
            )
        else:
    ...
    async def handle_message(
        ...
        assert message_text is not None, "Message text must be available"
    ...
    async def handle_subject_selection(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE, subject_name: str
    ) -> None:
        ...
        if subject_name not in subject_names:
            await update.message.reply_text(
                ...
                f"Please choose from: {', '.join(subject_names)}"
            )
            return

        assert subject_name is not None, "Subject name must be provided"
        ...
        if not topics:
            await update.message.reply_text(
                f"No topics found for subject '{subject_name}'. "
    ...
    async def handle_topic_selection(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE, topic_name: str
    ) -> None:
        ...
        assert session.selected_subject is not None, "Subject name must be provided"
        ...
        if topic_name not in topic_names:
            await update.message.reply_text(
                ...
                f"Please choose from: {', '.join(topic_names)}"
            )
            return
    ...
    async def generate_question(
        ...
        assert update.effective_user is not None, "User must be available"
        assert update.effective_chat is not None, "Chat must be available"
        assert update.message is not None, "Message must be available"
        ...
        assert session.selected_subject is not None, "Subject name must be provided"
        assert session.selected_topic is not None, "Topic name must be provided"

        paragraph = self.notes_service.get_random_paragraph(
            session.selected_subject, session.selected_topic
        )

        if not paragraph:
            await update.message.reply_text(
                f"No content found for topic '{session.selected_topic}' "
                f"in subject '{session.selected_subject}'. "
                "Please check your notes directory."
            )
            return
        ...
        try:
            ...
            await update.message.reply_text(
                "Sorry, I encountered an error while generating a question. "
                "Please try again."
            )
            return
    ...
    async def handle_answer(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE, user_answer: str
    ) -> None:
        ...
        if not session.current_question:
            await update.message.reply_text(
                "No active question. Use /study to start a new session."
            )
            return
        ...
        try:
            ...
            await update.message.reply_text(
                "Sorry, I encountered an error while scoring your answer. "
                "Please try again."
            )
            return
    ...
    async def handle_next_action(
        ...
        if action == "📚 Another Question":
            ...
            if session.selected_subject:
                ...
                session.selected_topic = None
                topics = self.notes_service.get_topics_for_subject(
